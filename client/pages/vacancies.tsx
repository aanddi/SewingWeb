...
const VacanciesListPage: NextPage<IRibbonResponse> = ({ vacancies, totalPages, countVacanciesReturn }) => {
  ...
  const handleLoadMore = async () => {
    ...
    // Fetch additional vacancies using the same query parameters
    ...
    // Update the vacancies state with the newly fetched vacancies
    // ...
  }
  ...
  return (
    <Meta title="Вакансии">
      <VacanciesList vacancies={vacancies} totalPages={totalPages} countVacanciesReturn={countVacanciesReturn} />
      {isLoading && <p>Loading more vacancies...</p>}
      {!isLoading && (
...
export const getStaticProps: GetStaticProps<IRibbonResponse> = async () => {
  try {
    const response = await VacancyService.getRibbon(1)
    const { vacancies, countVacanciesReturn, totalPages } = response.data

    return {
      props: {
        vacancies,
        countVacanciesReturn,
        totalPages,
      },
