			...
			<div className={styles.VCard}>
				<div className={styles.VCard__content}>
					...
					<div className={styles.VCard__company}>
						<div className={styles.VCard__company_svg}><Image src={mark} alt={'Компания подтверждена'}/></div>
						<div className={styles.VCard__company_name}>{props.company}</div>
					</div>
					<div className={styles.VCard__adress}>
						<div className={styles.VCard__adress_svg}><Image src={adress} alt={'Адресс'}/></div>
						<div className={styles.VCard__adress_name}>{props.adress}</div>
					</div>
					<div className={styles.VCard__button}>
						<div className={styles.VCard__button_respond}><Link href="vacancies/1">Посмотреть</Link></div>
						{
							checkNumber 
							? <div onClick={() => setCheckNumber(!checkNumber)} className={styles.VCard__button_checkphone}>{props.phone}</div>
							: <div onClick={() => setCheckNumber(!checkNumber)} className={[styles.VCard__button_checkphone, styles.VCard__button_checkphone_text].join(' ')}>Показать номер</div>
						}
					</div>
				</div>
			</div>
